...
function addInterceptor() {
  ...
  args.forEach(function (item) {
    if (!(0, _helpers.isFunc)(item)) {
      return (0, _helpers.error)('Spawn: the addInterceptor takes only a function as arguments!');
    }
  });
...
function createStore() {
  ...
  if (args.length === 1) {
    if ((0, _helpers.isPlainObject)(args[0])) {
      initialState = args[0];
      return new _core.Spawn(initialState, middlewares);
    } else {
      if ((0, _helpers.isArray)(args[0])) {
        middlewares = args[0].filter(function (item) {
          return (0, _helpers.isFunc)(item);
        });
        return new _core.Spawn(initialState, middlewares);
      }
      return (0, _helpers.error)('Spawn: createStore with one argument takes only a plain object or applyMidleware function!');
    }
  }
  if (args.length > 1) {
    if ((0, _helpers.isPlainObject)(args[0])) {
      ...
      return (0, _helpers.error)('Spawn: createStore with two arguments takes as first argument only a plain object!');
    }
    if ((0, _helpers.isArray)(args[1])) {
      middlewares = args[1].filter(function (item) {
        return (0, _helpers.isFunc)(item);
      });
      return new _core.Spawn(initialState, middlewares);
    }
    return (0, _helpers.error)('Spawn: createStore with two arguments takes as second argument only applyMidleware function!');
  }
...
var SPAWN_INIT = exports.SPAWN_INIT = '@@SPAWN/INIT';
...
var Spawn = function Spawn(initialState, interceptors) {
  ...
  var state = initialState,
      ...
      virtualState = {},
      subscribers = { '*': [] },
      lastAction = { data: {}, type: _constants.SPAWN_INIT };
  ...
  this.select = function (selector) {
    if ((0, _helpers.isString)(selector)) {
      switch (selector) {
        ...
        case '=>':
          return (0, _methods.clone)(lastAction);
        default:
    ...
    return (0, _helpers.error)('Spawn: the select method takes only a string or function as argument!');
  };

  this.detect = function (zone, cb) {
    if (!(0, _helpers.isString)(zone)) return (0, _helpers.error)('Spawn: the detect method takes only a string for first argument!');
    if (!(0, _helpers.isFunc)(cb)) return (0, _helpers.error)('Spawn: the detect method takes only a function for second argument!');
  ...
  this.update = function (zone, action) {
    if (!(0, _helpers.isString)(zone)) return (0, _helpers.error)('Spawn: the update method takes only a string for first argument!');
    if (!(0, _helpers.isPlainObject)(action)) return (0, _helpers.error)('Spawn: action must be a plain object!');
    if (!(0, _helpers.hasKey)(action, 'data')) return (0, _helpers.error)('Spawn: action must be has \'data\' key!');
    if (!(0, _helpers.hasKey)(action, 'type')) return (0, _helpers.error)('Spawn: action must be has \'type\' key!');

    interceptors.push(update);
    (0, _methods.applyInterceptors)(_this)(action)(interceptors);
        ...
        return function (action) {
          ...
          if (zone === '*') {
            if ((0, _helpers.isPlainObject)(action.data)) {
              ...
              prevState = {};
              virtualState = {};
              lastAction = (0, _methods.clone)(action);
              (0, _methods.autorun)(subscribers);

              return _this2;
            }

            return (0, _helpers.error)('Spawn: the update method takes only a plain object for replace full state! Check your update(\'*\') method.');
          }
