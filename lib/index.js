var Spawn = (function () {
  ...
  var SpawnCreator = function () {
    ...
    instance.update = function (zone, data) {
      ...
      if (_plainZoneValue(zone, state) !== _plainZoneValue(zone, virtualState)) {
    ...
    function _plainZoneValue(key, state) {
      return JSON.stringify(_findZoneValue(key, state));
    }
      ...
      for (key in subscribers) {
        if (subscribers.hasOwnProperty(key)) {
          ...
          if (zone.length < key.length && new RegExp('^' + '\\' + zone + '.', 'i').test(key)) {
            if (_plainZoneValue(key, prevState) !== _plainZoneValue(key, state)) {
              _mapSubscribers(subscribers[key]);
              continue;
            }
          }
